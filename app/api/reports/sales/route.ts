...
import { campusScope, canViewSalesReport, type SessionUser } from "@/lib/permissions";

export async function GET(req: Request) {
  const session = await auth();
  if (!session?.user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  if (!canViewSalesReport(session.user as SessionUser)) {
    return NextResponse.json({ error: "无权查看销售报表" }, { status: 403 });
  }
