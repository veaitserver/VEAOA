...
export async function validateTargets(
  ...
  args: { studentId: string; classroomId: string; teacherId: string },
...
export async function assertNoConflict(
  ...
  args: {
    ...
    teacherId: string; classroomId: string; studentId: string;
    excludeLessonId: string;
  },
): Promise<void> {
  const clash = await tx.scheduledLesson.findFirst({
    where: {
      ...
      OR: [{ teacherId: args.teacherId }, { classroomId: args.classroomId }, { studentId: args.studentId }],
    },
  });
  if (clash) {
    const dim = clash.teacherId === args.teacherId ? "老师"
      : clash.classroomId === args.classroomId ? "教室" : "学生";
